...
function TeamWorkspace({ controller, data, draft, baseline, busy, sessionId, locale, t, setDraft, setBaseline, run, setNotice, select, clone, remove, externalConflict, clearExternalConflict }: WorkspaceCommon & {
  ...
  const applyTemplate = async (kind: 'development' | 'review' | 'product'): Promise<void> => {
    ...
    const template = kind === 'development' ? {
      name: t('development'),
      note: t('templateNoteDevelopment'),
      roles: [
        ['Planner', t('templateRoleDevelopmentPlanner')],
        ['Implementer', t('templateRoleDevelopmentImplementer')],
        ['Reviewer', t('templateRoleDevelopmentReviewer')],
      ],
      executionMode: 'serial' as const,
    } : kind === 'review' ? {
      name: t('reviewTeam'),
      note: t('templateNoteReview'),
      roles: [
        ['Correctness', t('templateRoleReviewCorrectness')],
        ['Security', t('templateRoleReviewSecurity')],
        ['Tests', t('templateRoleReviewTests')],
      ],
      executionMode: 'parallel' as const,
    } : {
      name: t('productTeam'),
      note: t('templateNoteProduct'),
      roles: [
        ['Product', t('templateRoleProductProduct')],
        ['UX', t('templateRoleProductUx')],
        ['Engineer', t('templateRoleProductEngineer')],
      ],
      executionMode: 'parallel' as const,
    }
    await run(async () => {
