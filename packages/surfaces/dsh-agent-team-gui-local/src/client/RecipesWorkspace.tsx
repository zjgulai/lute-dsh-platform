...
export function RecipesWorkspace({ controller, data, busy, t, run, setNotice }: RecipesWorkspaceProps): ReactNode {
  ...
  const fetchRecipe = async (): Promise<void> => {
    ...
    // Generation guard (2026-09-10 fix): a slow remote fetch used to paint a
    // stale preview over a newer parse/file/reset. The fetch shares the
    // preview generation counter so every other producer invalidates it.
    const generation = (previewGeneration.current += 1)
    setLocalError(''); setPreviewBusy(true)
    try {
