  ...
  async run(command, args, { capture = false, timeoutMs = 300_000 } = {}) {
    ...
    return new Promise((resolveRun, reject) => {
      ...
      child.once('close', (code, signal) => {
        // `close` (not `exit`) so piped stdout/stderr finish draining before
        // we resolve: on macOS the exit event can beat the last data chunk
        // and yield empty capture (2026-09-10 fix, flaky smoke test).
        clearTimeout(timer)
        clearTimeout(forceTimer)
        if (timedOut) {
