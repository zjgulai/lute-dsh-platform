...
export interface Frontmatter {
  ...
  description?: string
  whenToUse?: string
  /** Business-facing one-line intro (human copy; model routing still uses description). */
  userSummary?: string
  /** Optional "try saying this" guided example shown on the card. */
  userTry?: string
  hint?: string
  recordInput?: boolean
...
export function parseFrontmatter(content: string): Frontmatter {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (/^[|>][-+]?$/.test(rest)) {
      ...
      else if (key === 'title') out.title = text || undefined
      else if (key === 'description') out.description = text || undefined
      else if (key === 'whenToUse') out.whenToUse = text || undefined
      else if (key === 'user_summary') out.userSummary = text || undefined
      else if (key === 'user_try') out.userTry = text || undefined
      continue
    }
    if (key === 'name') out.name = unquote(rest)
    else if (key === 'title') out.title = unquote(rest)
    else if (key === 'description') out.description = unquote(rest)
    else if (key === 'whenToUse') out.whenToUse = unquote(rest)
    else if (key === 'user_summary') out.userSummary = unquote(rest)
    else if (key === 'user_try') out.userTry = unquote(rest)
