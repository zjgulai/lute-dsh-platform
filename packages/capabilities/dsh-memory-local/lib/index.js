...
import { registerLingshuTools } from './tools.js';
import { installMemoryHooks } from './hooks.js';
/**
 * dbPath 默认值哨兵（D7/CWD 治理）：schema 层不允许在加载期解析绝对路径——
 * 那会锚定到主进程 cwd（Electron 下为 /）并被配置快照固化（平台案例#6）。
 * 默认在 apply 期才落到 ~/.dsh/data/lingshu.db。
 */
const DB_PATH_SENTINEL = '\0resolve-at-apply';
function resolveDbPath(raw) {
    if (raw !== DB_PATH_SENTINEL)
        return raw;
    return join(homedir(), '.dsh', 'data', 'lingshu.db');
}
/**
...
export const Config = z.object({
    ...
    python: z.string().default(process.platform === 'darwin' ? 'python3' : 'python'),
    moduleArgs: z.array(String).default(['-m', 'aeis.mcp.server']),
    dbPath: z.string().default(DB_PATH_SENTINEL),
...
export async function apply(ctx, config) {
    ...
    ctx.logger.info(`dsh-memory: 灵枢插件激活（大脑模式）· 接受护栏宪章 ${config.charter} —— ` +
        '接入即接受宪章约束（公开/可执行/可审计/设计者终裁）');
    // D7/CWD 治理：apply 期解析 dbPath（哨兵 → ~/.dsh/data/lingshu.db）
    const dbPath = resolveDbPath(config.dbPath);
    ctx.logger.info(`dsh-memory: 记忆库路径 ${dbPath}`);
    ...
    try {
        ...
        const tryRegister = async () => {
            if (toolsRegistered || !bridge.isReady())
                return;
            try {
        ...
        if (config.mutual.enabled) {
            ...
            if (!hasTimer) {
                ...
                installMutualMaintenance(ctx, { heartbeatMs: config.mutual.heartbeatMs }, {
                    verify: async (claim) => {
