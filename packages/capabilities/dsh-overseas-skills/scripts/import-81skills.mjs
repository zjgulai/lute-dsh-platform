...
/** 提取源 frontmatter 顶层单行标量（原样保留，值已 JSON 引号化） */
function extractTopLine(fm, key) {
...
function main() {
  ...
  for (const s of MAPPING.skills) {
    ...
    const userSummary = extractTopLine(fm, "user_summary");
    const userTry = extractTopLine(fm, "user_try");
    const out = [
      "---",
      `name: ${jstr(s.name)}`,
      `title: ${jstr(s.src)}`,
