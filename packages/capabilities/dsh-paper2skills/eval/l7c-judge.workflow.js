/**
 * F6 契约层实测 · 判分批（workflow 工具用的脚本体，留档以便复跑）
 *
 * 盲配对：判分 agent 只看 `<pair>/X.md` 与 `<pair>/Y.md`，看不到臂名 ——
 * X/Y 的位置由 case 奇偶决定（eval/prepare-l7c-judging.mjs），位置偏差成对抵消。
 *
 * 三维固定量表（与原 L7 实验同名的三维；**逐维定义写死在这里，不许跑完再调**）：
 ...
 * 各 0–5 整数分。判官是 LLM，原实验实测噪声 ±0.6~0.7（满分 15）。
 *
