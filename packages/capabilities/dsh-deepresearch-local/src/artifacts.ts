...
export function readableFetchText(body: string): string {
  const raw = body.trim()
  if (raw === '') return ''
...
export async function persistArtifact(scope: { projectId: string; questionId: string; criterionId: string; url: string; body: string; rootDir?: string }): Promise<PersistedArtifact | null> {
