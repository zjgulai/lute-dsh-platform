...
export class DeepResearchService extends TypertRemoteService {
  ...
  static Config: s<Config> = s.object({
    ...
    // D7/CWD governance (2026-09-10): empty-string sentinel replaces the
    // load-time process.cwd() default (Electron resolves it to /); the
    // consumption point resolves it at apply time to ~/.dsh/data/research-runner.
  ...
  private async runCriterion(id: ResearchId, run: ActiveResearchRun, questionId: ResearchQuestionId, criterionId: string, dependencyContext: string): Promise<void> {
    ...
    for (const candidate of accepted) {
      ...
      await this.addEvidence({
        ...
        // (2026-09-10 fix) A silently swallowed rejection (evidence limit etc.)
        // skewed the coverage stats with no trace: warn loudly and write the
        // rejection reason into the criterion's warning field.
  ...
  private async runWriting(id: ResearchId, run: ActiveResearchRun): Promise<void> {
    ...
    await this.spawnRole(id, run, {
      ...
      onDraft: text => {
        void this.update(id, current => current.phase === 'writing' ? { ...current, report: text } : current).catch(err => this.ctx.logger.warn(`deepresearch: report write-back failed: ${String(err)}`))
      },
    })
  }
  ...
  private async spawnRole(_id: ResearchId, run: ActiveResearchRun, spec: {
    ...
    const handle = await this.ctx.agents.create({
      ...
      meta: { cwd: resolve(this.config.runnerCwd === '' ? join(homedir(), '.dsh', 'data', 'research-runner') : this.config.runnerCwd), origin: 'subagent', delegationDepth: 1 },
