...
import { BridgeServer, isLoopbackAddress } from "./server.js";
...
import { makeApiProxy, toFetchHandler } from "./apiproxy-shim.js";
...
export async function apply(ctx, config) {
    ...
    const api = withSessionDeferral(withSessionWorkspace(makeApiProxy(gateway), resolved.sessionWorkspacePath, message => { ctx.logger.warn(message); }), resolved.deferSessionCreate, ctx.get('attachments')?.imageLimits);
    ...
    const purgeSession = async (sessionId) => {
        ...
        const deps = { sessionsRoot: SESSIONS_ROOT, runningSessionIds };
