...
import type { MuxFrame, RpcRequest } from '../src/apiproxy-shim.js'
import { RpcId } from '../src/apiproxy-shim.js'
...
describe('BridgeServer', () => {
  ...
  it('terminates a half-open slot that never answers pongs (2026-09-10 fix)', async () => {
    // Fast cadence: ping every 40ms, give up 120ms after the last pong.
    const h = await startBridge({ pingIntervalMs: 40, pongTimeoutMs: 120 })
    harnesses.push(h)
    const { ws, frames, done } = await connect(h.url)
    send(ws, { t: 'hello', token: TOKEN, caps: CAPS })
    await waitFor(() => frames.some((f) => f.t === 'hello.ok'))
    // Silently drop every ping (no pong) — the connection must be terminated
    // instead of keeping the single slot busy forever.
    await done
    // The server-side close handler runs on a later tick than the peer's
    // close notification; wait for the slot to actually clear.
    await waitFor(() => !h.bridge.hasConnection(), 2_000)
    expect(h.bridge.hasConnection()).toBe(false)
    // And a fresh connection can immediately take over the slot.
    const again = await connect(h.url)
    send(again.ws, { t: 'hello', token: TOKEN, caps: CAPS })
    await waitFor(() => again.frames.some((f) => f.t === 'hello.ok'))
    expect(h.bridge.hasConnection()).toBe(true)
    again.ws.close()
  })

  it('keeps a slot alive while pongs keep arriving', async () => {
    const h = await startBridge({ pingIntervalMs: 40, pongTimeoutMs: 120 })
    harnesses.push(h)
    const { ws, frames } = await connect(h.url)
    send(ws, { t: 'hello', token: TOKEN, caps: CAPS })
    await waitFor(() => frames.some((f) => f.t === 'hello.ok'))
    // Answer every ping with a pong for ~360ms (3x the timeout window).
    ws.on('message', () => {
      if (frames.some((f) => f.t === 'ping') && frames.filter((f) => f.t === 'ping').length > frames.filter((f) => f.t === 'pong').length) {
        send(ws, { t: 'pong' })
      }
    })
