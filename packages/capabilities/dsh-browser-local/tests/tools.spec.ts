...
describe('registerBrowserTools', () => {
  ...
  it('refuses to dispatch browser_navigate for non-http(s) URLs (host-side allowlist, 2026-09-10)', async () => {
    const { ctx, bridge, requestTool, registered } = makeHarness()
    const disposers = registerBrowserTools(ctx, bridge, { toolTimeoutMs: 1_000, snapshotMaxChars: 12_000, maxInteractiveItems: 60 })
    const navigate = registered.find((r) => r.name === 'browser_navigate')
    expect(navigate).toBeDefined()
    const exec = { signal: new AbortController().signal } as never
    const run = (url: unknown) => (navigate!.definition as { execute: (args: Record<string, unknown>, exec: unknown) => Promise<{ text: string }> }).execute({ url }, exec)
    for (const bad of ['file:///etc/passwd', 'about:blank', 'chrome-extension://abc/popup.html', 'javascript:alert(1)', 'ws://127.0.0.1/x', '']) {
      const result = await run(bad)
      expect(result.text).toContain('browser_navigate refused')
    }
    // Every refusal happened without a single dispatch to the extension.
    expect(requestTool).not.toHaveBeenCalled()
    // Correct URLs still dispatch untouched (trimmed once, then forwarded).
    const ok = await run('  https://example.com/a?b=c  ')
    expect(ok.text).toBe('ok')
    expect(requestTool).toHaveBeenLastCalledWith('browser_navigate', { url: 'https://example.com/a?b=c' }, expect.anything(), 1000)
    for (const dispose of disposers.values()) dispose()
  })
