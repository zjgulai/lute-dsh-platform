...
import { makeApiProxy, toFetchHandler, type ApiProxy } from './apiproxy-shim.ts'
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  const gateway = ctx.typertGateway as unknown as BridgeTypertGateway
  ...
  const purgeSession = async (sessionId: string): Promise<void> => {
    ...
    try {
      const listed = await api.sessions.list({ rpcId: randomUUID(), payload: {} })
      if (listed.result.ok) {
      ...
      // Fail-closed (2026-09-10 fix): when the running list is unavailable we
      // cannot prove this session is NOT running — treat it as running so the
      // purge refuses instead of potentially deleting a live session's files.
  ...
  const server = new BridgeServer({
    ...
    openEvents: (signal) => api.events.mux({ rpcId: randomUUID(), payload: {} }, signal),
  ...
  const route = {
    path: BRIDGE_PATH,
    handler: (req: any, socket: any, head: Buffer) => { server.handleUpgrade(req, socket, head) },
  } as { path: string; handler: (req: any, socket: any, head: Buffer) => void }
  ...
  const configRoute = {
    ...
    // Zero-config discovery answers loopback peers only (2026-09-10 fix):
    // the payload names the internal WS URL; on a 0.0.0.0 deployment that is
    // internal topology a remote host has no business reading.
    handler: (req: { socket?: { remoteAddress?: string }; headers?: Record<string, string | string[] | undefined> }, res: { writeHead: (code: number, headers: Record<string, string>) => void; end: (body: string) => void }) => {
  ...
  } as { kind: 'exact'; path: string; handler: (req: unknown, res: { writeHead: (code: number, headers: Record<string, string>) => void; end: (body: string) => void }) => void }
