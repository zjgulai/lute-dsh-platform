...
function plainFail(message: string): never {
 ...
 */
const install = (_ctx: Context, fail: (message: string) => void = plainFail): void => {
  if (!(HELLO_TIMEOUT_MS > 0)) fail('hello authentication timeout must be positive')
  if (!isLoopbackAddress('127.0.0.1')) fail('loopback classifier must accept 127.0.0.1')
  if (!isLoopbackAddress('::ffff:127.0.0.1')) fail('loopback classifier must accept IPv4-mapped loopback')
  if (isLoopbackAddress('8.8.8.8')) fail('loopback classifier must reject public addresses')
  if (isLoopbackAddress(undefined)) fail('loopback classifier must treat missing address as non-loopback')
}
...
export const apply = (ctx: Context): Promise<() => void> =>
  Promise.resolve(ctx.invariants.register('@yuxianglin/dsh-bridge-browser', install))
