...
function defineTools(call: Call, options: BrowserToolsOptions): DefinedTool[] {
  ...
  const navigate = (): DefinedTool => defineTool({
    ...
    output: TEXT_OUTPUT,
    execute: (args, exec) => {
      // Host-side defense in depth (2026-09-10): the parameter schema cannot
      // express a protocol predicate, so the host rejects non-http(s) targets
      // (file://, about:, chrome-extension:// …) before dispatching the frame;
      // the extension keeps its own checks as the outer layer.
      const url = typeof args.url === 'string' ? args.url.trim() : ''
      if (!/^https?:\/\//i.test(url)) {
        return Promise.resolve<TextResult>({
          text: `browser_navigate refused: url must be an http(s) URL. Host-side allowlist rejected ${JSON.stringify(args.url ?? null)}; file:// and other schemes are not dispatched to the extension.`,
        })
      }
