...
export interface MuxFrame { type: string; event: unknown; eventId?: string }
...
export interface Envelope<T = unknown> { result: RpcResult<T> }
...
export function toEndpoint(method: string): string {
...
export interface ApiProxy {
  sessions: {
    list(req: RpcRequest<Record<string, never>>): Promise<Envelope<{ items: Array<{ sessionId: SessionId; running: boolean }> }>>
    create(req: RpcRequest<{ workspaceId?: WorkspaceId; cwd?: string; sessionId?: SessionId; [key: string]: unknown }>): Promise<Envelope<{ sessionId: SessionId }>>
    history(req: RpcRequest<{ sessionId: SessionId }>): Promise<Envelope<unknown>>
    prompt(req: RpcRequest<{ sessionId: SessionId; [key: string]: unknown }>): Promise<Envelope<unknown>>
  }
  workspace?: {
    create(req: RpcRequest<{ path: string }>): Promise<Envelope<{ workspace: { workspaceId: WorkspaceId } }>>
  }
  events: {
...
export function makeApiProxy(gateway: BridgeTypertGateway): ApiProxy {
  ...
  const rpc = (endpoint: string) => (payload: unknown): Promise<Envelope> => {
    const [ns, m] = endpoint.split('/')
    return gateway
      .invoke({ namespace: ns!, method: m!, args: { request: payload } }, new AbortController().signal)
      .then((value) => ({ result: { ok: true as const, value } }))
      .catch((error: unknown) => {
        const e = error as { code?: string; message?: string; cause?: unknown } | Error
        ...
        return { result: { ok: false as const, error: { code: (e as { code?: string }).code ?? 'internal', message } } }
      })
  }
  return {
    sessions: {
      list: rpc('session/list') as ApiProxy['sessions']['list'],
      create: rpc('session/create') as ApiProxy['sessions']['create'],
      ...
      history: (async (req: RpcRequest<{ sessionId: SessionId }>): Promise<Envelope<{ events: Array<{ event: unknown }>; hasMore?: boolean; projections?: unknown }>> => {
        ...
        try {
          ...
          if (listed.ok === true && listed.value !== undefined) {
            const items = (listed.value as { items?: Array<{ sessionId?: string; projections?: { asOfSeq?: number } }> }).items
            const item = Array.isArray(items) ? items.find((entry) => entry.sessionId === sessionId) : undefined
            if (item?.projections !== undefined && typeof item.projections.asOfSeq === 'number') throughSeq = item.projections.asOfSeq
          }
        } catch {
          /* best-effort cursor: -1 yields an empty page instead of failing */
        }
        let pageResult: { ok: boolean; value?: unknown; error?: { code: string; message: string } }
        try {
          const value = await gateway.invoke(
            { namespace: 'session', method: 'page', args: { request: { address: { kind: 'session', sessionId }, throughSeq } } },
          ...
          pageResult = { ok: false, error: { code: (e as { code?: string }).code ?? 'internal', message } }
        }
        if (pageResult.ok !== true) {
          return { result: pageResult as unknown as RpcResult<{ events: Array<{ event: unknown }>; hasMore?: boolean; projections?: unknown }> }
        }
        const value = pageResult.value as {
        ...
        const reshaped: { events: Array<{ event: unknown }>; hasMore?: boolean; projections?: unknown } = {
          events,
          hasMore: value.hasMore ?? false,
        }
        if (value.projections !== undefined) reshaped.projections = value.projections
        return { result: { ok: true, value: reshaped } }
      }) as ApiProxy['sessions']['history'],
      // alpha requires a requestId on session/prompt; the rc panel never sends one.
      prompt: ((req: RpcRequest<{ sessionId: SessionId; [key: string]: unknown }>) => {
        const payload = { ...req.payload }
        if (payload.requestId === undefined) payload.requestId = randomUUID()
        return rpc('session/prompt')(payload) as ReturnType<ApiProxy['sessions']['prompt']>
      }) as ApiProxy['sessions']['prompt'],
    },
    workspace: { create: rpc('workspace/create') as NonNullable<ApiProxy['workspace']>['create'] },
    events: {
      async *mux(_req: RpcRequest<Record<string, never>>, signal: AbortSignal): AsyncIterable<RpcRequest<MuxFrame>> {
        const stream = gateway.openWireStream('$events', { args: {} }, signal) as AsyncIterable<{
          type?: string
          event?: string
          args?: unknown
          eventId?: string
        ...
        for await (const frame of stream) {
          ...
          const payload: MuxFrame = { type: frame.event ?? 'session/event', event: frame.args }
          if (frame.eventId !== undefined) payload.eventId = frame.eventId
          ...
          if (frame.type === 'waterfall' && frame.eventId !== undefined && clientId !== undefined) {
            gateway
              .dispatchRpc(
                '$events/result',
                { args: { clientId, eventId: frame.eventId, outcome: { kind: 'next' } } },
                signal,
              )
              .catch(() => {})
          }
 ...
 */
export function toFetchHandler(api: ApiProxy, gateway: BridgeTypertGateway): {
  ...
  return {
    fetch: async (request: Request): Promise<Response> => {
      ...
      let body: { method?: string; payload?: unknown; rpcId?: string } = {}
      try {
      ...
      try {
        ...
        const sessionsApi = api.sessions as unknown as Record<string, (req: RpcRequest<unknown>) => Promise<Envelope>>
        const workspaceApi = api.workspace as unknown as Record<string, (req: RpcRequest<unknown>) => Promise<Envelope>> | undefined
        if (rcNs === 'session' && rcM !== undefined && typeof sessionsApi[rcM] === 'function') {
          ...
          envelope = { result: result as RpcResult }
        }
