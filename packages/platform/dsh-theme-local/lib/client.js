window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region \0dsh-theme-css:/Users/lute/project/Magpie-Horch/dsh-theme-local/src/client/studio.css.mjs
		...
		//#endregion
		//#region ui-fixes.css（LUTE 2026-09-11：composer 底行防挤压 + log 按钮缩小）
		const uiFixesCss = [
			"/* composer 底行防挤压：长模型名不再把「完全权限/附件」簇挤到上一行（稳定锚点 data-composer-card，无哈希依赖） */",
			"[data-composer-card] > div:last-child { flex-wrap: nowrap; }",
			"[data-composer-card] > div:last-child > div { min-width: 0; }",
			"[data-composer-card] > div:last-child > div:first-child { flex: 0 1 auto; }",
			"[data-composer-card] > div:last-child > div:nth-child(2) { flex: 0 1 auto; }",
			"/* 行内按钮一律不收缩、不换行（继承/附件/发送/命令等） */",
			"[data-composer-card] > div:last-child button,",
			"[data-composer-card] > div:last-child [data-slot=\"conversation.input.right\"],",
			"[data-composer-card] > div:last-child [data-slot=\"conversation.input.left\"] { white-space: nowrap; flex: none; }",
			"/* 只有模型触发器和权限 select 吸收挤压（自带省略号） */",
			"[data-composer-card] > div:last-child .Ns6z9q_root,",
			"[data-composer-card] > div:last-child select { flex: 0 1 auto; min-width: 0; }",
			"[data-composer-card] > div:last-child select { max-width: 160px; overflow: hidden; text-overflow: ellipsis; }",
			"",
			"/* log 按钮：缩小为 1/2 + 去外边框（真实类名 Bljk-W_sessionLogButton + [data-log-clone] 版本无关 + pTsq1a 旧哈希兜底） */",
			"[data-log-clone], .Bljk-W_sessionLogButton, .pTsq1a_sessionLogButton {",
			...
			"  border-radius: 8px;",
			"}",
			"[data-log-clone] svg, .Bljk-W_sessionLogButton svg, .pTsq1a_sessionLogButton svg { width: 10px !important; height: 10px !important; }"
		].join("\n");
		const uiFixesTagId = "dsh-theme/ui-fixes.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(uiFixesTagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-theme";
			tag.dataset.pluginCss = uiFixesTagId;
			tag.textContent = uiFixesCss;
			document.head.appendChild(tag);
		}
		//#endregion
