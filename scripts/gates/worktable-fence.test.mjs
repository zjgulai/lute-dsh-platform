 ...
 * @returns {{root: string, profileDir: string}}
 */
function prepareTree(fixture, mutate = () => {}) {
  ...
  const profileDir = cleanProfile(fixture.profile)
  mutate({ root, profileDir })
  return { root, profileDir }
}

const run = (mutate) => withMutationFixture({
  prefix: 'worktable-fence',
  prepare(fixture) { prepareTree(fixture, mutate) },
}, async (fixture) => {
    const tree = { root: fixture.repo, profileDir: join(fixture.profile, 'profile') }
...
test('卸载门禁：干净的最小树必须通过', { skip: !assetsAvailable && '资产缺失' }, async () => {
  assert.equal((await run()).passed, true)
})

test('卸载门禁：dependencies 里又有它必须被拒绝', { skip: !assetsAvailable && '资产缺失' }, async () => {
  const result = await run(({ profileDir }) => {
...
test('卸载门禁：bundles 里又有它必须被拒绝', { skip: !assetsAvailable && '资产缺失' }, async () => {
  const result = await run(({ profileDir }) => {
    const pkg = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8'))
...
test('卸载门禁：node_modules 里的残留必须被拒绝（实测过的那个状态）', { skip: !assetsAvailable && '资产缺失' }, async () => {
...
test('卸载门禁：资产缺失（补丁被删）必须被拒绝', { skip: !assetsAvailable && '资产缺失' }, async () => {
...
test('卸载门禁：资产缺失（pin 被删）必须被拒绝', { skip: !assetsAvailable && '资产缺失' }, async () => {
  const result = await run(({ root }) => {
...
test('卸载门禁：pin 与 vendor HEAD 不一致必须被拒绝', { skip: !assetsAvailable && '资产缺失' }, async () => {
  const result = await run(({ root }) => {
    const vendor = join(root, 'vendor', 'dsh-worktable')
    mkdirSync(vendor, { recursive: true })
    writeFileSync(join(vendor, 'README.md'), 'temporary vendor fixture\n')
    execFileSync('git', ['-C', vendor, 'init', '--quiet'])
    execFileSync('git', ['-C', vendor, 'config', 'user.name', 'QG-006A Fixture'])
    execFileSync('git', ['-C', vendor, 'config', 'user.email', 'fixture@example.invalid'])
    execFileSync('git', ['-C', vendor, 'add', 'README.md'])
    execFileSync('git', ['-C', vendor, '-c', 'commit.gpgSign=false', 'commit', '--quiet', '-m', 'fixture'])
    const pinFile = join(root, 'vendor', 'dsh-worktable.pin')
    writeFileSync(pinFile, readFileSync(pinFile, 'utf8').replace(/^upstream-sha:\s*\S+/m, `upstream-sha: ${'0'.repeat(40)}`))
  })
  assert.equal(result.passed, false)
  assert.ok(mentions(result.violations, '≠ pin 的'), JSON.stringify(result.violations))
})

test('卸载门禁：vendor 未 clone 时跳过资产项，但安装面三项仍须照判', async () => {
...
test('卸载门禁：vendor 不在且安装面干净时报跳过而不是失败', async () => {
