...
export class LingshuBridge {
    ...
    spawnAndHandshake() {
        ...
        proc.stderr?.on('data', (chunk) => {
            ...
            if (text)
                console.error(`[lingshu-bridge] ${text}`);
        });
    ...
    async handshake() {
        try {
            ...
            if (!this.disposed) {
                console.error(`[lingshu-bridge] 握手失败: ${err.message}`);
    ...
    scheduleRetry() {
        ...
        if (this.gaveUp) {
            ...
            this.readyState = 'failed';
            this.flushBootQueue(false);
            console.error(`[lingshu-bridge] 连续启动失败 ${this.retries} 次，已停止自动重启（不再后台空转）。` +
    ...
    request(method, params, signal) {
        ...
        if (this.gaveUp) {
            return Promise.reject(new Error('灵枢进程不可用：连续启动失败已达上限，已停止重试。' +
