...
- **后续动作（未完成，如实记录）**：
  1. **本机在身份版上的一次性重授尚未完成**——判据④/⑤ 的运行时读数因此仍空着。重授后必须看
     **要求形态**（`certificate leaf` 才算 ⑤ 具备成立条件；仍是 `cdhash` 即 ADR-0063 路线在自签前提下
     失效），再升级到 2.3.2 取 `--diff`。
  2. **「输入监控」是否真为必需未定**（实验方法见上）。未定之前指引保持三项。
