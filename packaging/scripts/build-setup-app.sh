...
# 用法: ./build-setup-app.sh <输出目录> <版本> <基线>   （产出 <输出目录>/LUTE Setup.app）
#
# 版本是**必填**的，并写进向导自己的 CFBundleVersion（`<基线>-lute.<版本>`，与出货 app 同一约定）：
...
# <基线>（如 2.0.5 / 2.0.10）由 assemble.sh 从 staged app 的 ShortVersionString 派生后传入，
# 本脚本不硬编码任何一代基线（2026-09-17：2.0.5 字面会让 2.0.10 世代的向导谎报基线）。
set -euo pipefail
PKG_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="${1:?用法: build-setup-app.sh <输出目录> <版本> <基线>}"
SETUP_VERSION="${2:?用法: build-setup-app.sh <输出目录> <版本> <基线>（向导版本必须与载荷一致）}"
SETUP_BASELINE="${3:?用法: build-setup-app.sh <输出目录> <版本> <基线>（取 staged app 的 ShortVersionString）}"
case "$SETUP_BASELINE" in
  ''|*[!0-9.]*) echo "[setup] ✗ 非法基线版本：${SETUP_BASELINE}" >&2; exit 1 ;;
esac
...
<dict>
  ...
  <key>CFBundleVersion</key><string>${SETUP_BASELINE}-lute.${SETUP_VERSION}</string>
  <key>CFBundleShortVersionString</key><string>${SETUP_BASELINE}</string>
